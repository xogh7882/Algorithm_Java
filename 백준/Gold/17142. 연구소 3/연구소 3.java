...
public class Main{
	...
	private static void BFS() {
		...
		for(int i=0;i<list.size();i++) {
			visited[list.get(i)[0]][list.get(i)[1]] = 2;
		}
		
		int copycount = count;
		copycount -= (virusNum - M);
		
		Queue<int[]> queue = new ArrayDeque<>();
		for(int i=0;i<M;i++) {
			queue.offer(list.get(select[i]));
			visited[list.get(select[i])[0]][list.get(select[i])[1]] = 3;
		}
		...
		while(!queue.isEmpty()) {
			...
			for(int i=0;i<4;i++) {
				int nr = r + dr[i];
				int nc = c + dc[i];
				if(check(nr,nc) && visited[nr][nc] == 2 && map[nr][nc] == 2) {
					visited[nr][nc] = 4;
